...
import { fetchTrucks } from "./service";
import { Truck } from "lucide-react";
import SideBar from "../../common-shared/side-bar/side-bar";
import { deleteTrucks } from "./service";
...
const Trucks = () => {
  ...
  const onDeleteClicked = async (ids: string[]) => {
    const result = await deleteTrucks(ids);
    if (result) {
  ...
  const columns = [
    { headerName: "Number Plate", field: "number_plate", width: 200 },
    { headerName: "Status", field: "status", width: 150 },
    { headerName: "Capacity (L)", field: "capacity_liters", width: 150 },
    ...
    <>
      <div className="h-full w-full bg-slate-50 flex flex-col overflow-hidden">
        <div className="w-full flex flex-col p-4 sm:p-6  overflow-hidden">
          <PageHeader
            title="Trucks"
            description="Manage all trucks in one place. Add new trucks, update their details, and assign them to routes to keep your waste collection running smoothly."
            Icon={Truck}
          />
